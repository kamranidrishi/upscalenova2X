...
const content = fs.readFileSync('src/components/GymDemo.tsx', 'utf-8');

if (!content.includes('mega-theme')) {
    // Inject the mega-theme class to the root wrapper
    const newContent = content.replace(
        /<div className="w-full h-full bg-\[#060B14\]/g,
        '<div className={`w-full h-full bg-[#060B14] ${isMega ? "mega-theme" : ""}`}'
    );
    
    // Inject the CSS block right inside the wrapper
    const cssBlock = `
      {isMega && (
        <style dangerouslySetInnerHTML={{__html: \`
          ...
          .mega-theme .to-indigo-600 { --tw-gradient-to: #78350F !important; }
          ...
          .mega-theme .border-slate-800 { border-color: rgba(212, 175, 55, 0.15) !important; }
          .mega-theme .border-slate-700 { border-color: rgba(212, 175, 55, 0.25) !important; }
    ...
    // insert right after the wrapper div opening
    const finalContent = newContent.replace(
        /(<div className=\{`w-full h-full bg-\[#060B14\] \$\{isMega \? "mega-theme" : ""\}`\}>)/,
        `$1\n${cssBlock}`
    );
    
    fs.writeFileSync('src/components/GymDemo.tsx', finalContent);
    console.log('Applied CSS overrides to GymDemo.tsx');
} else {
