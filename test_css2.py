...
if 'mega-theme' not in content:
    ...
    old_wrapper = '<div className="w-full h-full bg-[#060B14] text-slate-100 overflow-y-auto overflow-x-hidden font-sans custom-scrollbar select-none">'
    
    new_wrapper = '<div className={`w-full h-full bg-[#060B14] text-slate-100 overflow-y-auto overflow-x-hidden font-sans custom-scrollbar select-none ${isMega ? "mega-theme" : ""}`}>'
...
"""
    if old_wrapper in content:
        ...
        with open('src/components/GymDemo.tsx', 'w') as f:
            f.write(content)
        print("Success: Replaced wrapper and added CSS.")
    else:
        print("Error: Could not find wrapper string in GymDemo.tsx")
else:
    print("Already applied mega-theme")
